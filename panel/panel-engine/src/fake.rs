...
impl FakeGatewayEngine {
    ...
    fn validate_snapshot(&self, snapshot: &RuntimeSnapshot) -> Result<ValidationReport> {
        ...
        validate_ir_references(snapshot, &mut diagnostics);
        let unsupported: Vec<_> = snapshot
            .required_capabilities()
            .iter()
            .filter(|required| {
                !self.capabilities.contains(&EngineCapability::new(
                    required.name.clone(),
...
fn validate_ir_references(snapshot: &RuntimeSnapshot, diagnostics: &mut Vec<Diagnostic>) {
    use panel_ir::RouteAction;
